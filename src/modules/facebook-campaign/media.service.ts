...
export class MediaService {
  ...
  async createPresignedUpload(
    ...
    const mediaType =
      dto.mediaType === 'video' ? MediaType.VIDEO : MediaType.IMAGE;
    ...
    const media = await this.mediaRepository.save({
      ...
      uploadStatus: MediaStatus.UPLOADING,
      metaImageHash: null,
      metaVideoId: null,
      errorMessage: null,
    });
  ...
  async completeUpload(
    ...
    if (!media.storageUrl?.startsWith('https://')) {
      media.uploadStatus = MediaStatus.FAILED;
  ...
  async recordServerUpload(params: {
    ...
    if (!params.storageUrl?.startsWith('https://')) {
      throw new BadRequestException(
        'Media public URL must use HTTPS so Meta can download it later.',
      );
    }
    ...
    return this.mediaRepository.save({
      ...
      mediaType:
        params.mediaType === 'video' ? MediaType.VIDEO : MediaType.IMAGE,
  ...
  async findByStorageUrl(
    storageUrl: string,
  ): Promise<MetaCampaignMedia | null> {
  ...
  async findMetaRefsByStorageUrl(
    storageUrl: string,
  ): Promise<{ metaImageHash: string | null; metaVideoId: string | null } | null> {
    const media = await this.findByStorageUrl(storageUrl);
  ...
  async findReadyMetaRefsByUrl(
    ...
    return this.findMetaRefsByStorageUrl(storageUrl);
  }

  async markMetaUploadSuccess(
    storageUrl: string,
    ...
    const patch: Partial<MetaCampaignMedia> = {
      uploadStatus: MediaStatus.META_UPLOADED,
      errorMessage: null,
    };
  ...
  async markMetaUploadFailed(
    ...
    const media = await this.findByStorageUrl(url);
    if (!media) return;
    if (media.uploadStatus === MediaStatus.META_UPLOADED) return;

    media.uploadStatus = MediaStatus.FAILED;
    media.errorMessage = errorMessage.slice(0, 2000);
    await this.mediaRepository.save(media);
  }
  ...
  async markMetaRefs(
    ...
    await this.markMetaUploadSuccess(storageUrl, refs);
  }

  normalizeType(media: MetaCampaignMedia): 'image' | 'video' {
    return toStorageMediaType(media.mediaType);
  }
...
export { MediaService as MetaCampaignMediaService };
